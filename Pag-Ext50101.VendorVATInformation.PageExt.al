/*
Vendor Card 에 한국 부가세 매출입력을 위한 부가 정보를 입력하도록 합니다.
해당 정보는 VAT Basic Information 테이블에 저장됩니다.
*/
pageextension 50101 "Vendor VAT Information" extends "Vendor Card"
{
    layout
    {
        addlast(content)
        {
            group(VATInformation)
            {
                ...
                {
                    ToolTipML = KOR = '거래처 유형을 선택합니다.';
                    CaptionML = ENU = 'Vendor Type', KOR = '거래처 유형';
                    ApplicationArea = All;
                    Importance = Promoted;
                    trigger OnValidate()
                    begin
    ...
    begin
        ...
        VATBasicInformation.SetRange("Table ID", 23);//Vendor Table
        ...
        IF VATBasicInformation.Find('-') then begin
            ...
            VATBasicInformation."Table ID" := 23; //Vendor Table
    ...
    */
    ...
    begin
        ...
        VATInforFieldTableIDRef.Value := 23; //Vendor Table
